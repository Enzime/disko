      ...
      in
      {
        options = {
          ...
          _scripts = lib.mkOption {
            ...
            '';
            default = { pkgs, checked ? false }: let
              packages = cfg.config._packages pkgs;
            in {
              destroyScript = lib.writeShellApplication {
                name = "disko-destroy";
                ...
                text = cfg.config._destroy;
              };

              formatScript = lib.writeShellApplication {
                name = "disko-format";
                runtimeInputs = [ packages ];
                text = cfg.config._create;
              };

              mountScript = lib.writeShellApplication {
                name = "disko-mount";
                runtimeInputs = [ packages ];
                text = cfg.config._mount;
              };

              diskoScript = lib.writeShellApplication {
                name = "disko";
                runtimeInputs = [ packages pkgs.bash ];
                text = cfg.config._disko;
              };
